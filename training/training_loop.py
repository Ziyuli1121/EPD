...
def training_loop(
    ...
    **kwargs,
    ...
    while True:
        ...
        latents = [loss_fn.sigma_max * torch.randn([batch_gpu, net.img_channels, net.img_resolution, net.img_resolution], device=device) for k in range(num_accumulation_rounds)]
        labels = c = uc = [None for k in range(num_accumulation_rounds)]

        if net.label_dim:
            if guidance_type == 'cg':                                           # ADM models
                labels = [torch.randint(net.label_dim, size=(batch_gpu,), device=device) for k in range(num_accumulation_rounds)]
            elif guidance_type == 'cfg' and dataset_name in ['ms_coco']:        # Stable Diffusion (SD) models
                prompts = [random.sample(sample_captions, batch_gpu) for k in range(num_accumulation_rounds)]
                if guidance_rate != 1.0:
                    uc = [net.model.get_learned_conditioning(batch_gpu * [""]) for k in range(num_accumulation_rounds)]

                with torch.no_grad():
                    if isinstance(prompts[0], tuple):
                        prompts = [list(p) for p in prompts]
                    c = [net.model.get_learned_conditioning(prompts[k]) for k in range(num_accumulation_rounds)]
            else:                                                               # EDM models
                labels = [torch.eye(net.label_dim, device=device)[torch.randint(net.label_dim, size=[batch_gpu], device=device)] for k in range(num_accumulation_rounds)]
            ...
            if guidance_type in ['uncond', 'cfg']:      # LDM and SD models
                with autocast("cuda"):
                    with net.model.ema_scope():
                        teacher_traj = [loss_fn.get_teacher_traj(net=net, tensor_in=latents[k], labels=labels[k], condition=c[k], unconditional_condition=uc) for k in range(num_accumulation_rounds)]
            else:
                teacher_traj = [loss_fn.get_teacher_traj(net=net, tensor_in=latents[k], labels=labels[k]) for k in range(num_accumulation_rounds)]
        ...
        for step_idx in range(loss_fn.num_steps - 1):
            ...
            for round_idx in range(num_accumulation_rounds):
                with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                    if guidance_type in ['uncond', 'cfg']:      # LDM and SD models
                        with net.model.ema_scope():
                            loss, str2print, stu_out = loss_fn(predictor=ddp, net=net, tensor_in=latents[round_idx], 
                                                    labels=labels[round_idx], step_idx=step_idx, teacher_out=teacher_traj[round_idx][step_idx], 
                                                    condition=c[round_idx], unconditional_condition=uc[round_idx], dataset = dataset_name)
                    else:
                        loss, str2print, stu_out = loss_fn(predictor=ddp, net=net, tensor_in=latents[round_idx], 
                                                labels=labels[round_idx], step_idx=step_idx, teacher_out=teacher_traj[round_idx][step_idx], dataset = dataset_name)
